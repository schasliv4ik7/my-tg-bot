...
# Ставим False, чтобы делать запросы напрямую во избежание SSL Handshake ошибок
USE_PROXY = False
...
def get_sofascore_live():
    # Возвращаем рабочий адрес SofaScore, который использовался ранее
    ...
    try:
        ...
        print(f"[SofaScore Сеть] Делаем запрос к Live API ({connection_type})...", flush=True)
        ...
        print(f"[SofaScore Сеть] Ошибка подключения: {e}", flush=True)
        return None


def monitor_sofascore():
    time.sleep(5)
    print("=== [ПОТОК] СТАРТ МОНИТОРИНГА SOFASCORE ===", flush=True)
    send_telegram_message("📱 <b>Бот мониторинга SofaScore запущен напрямую!</b>\nПроверяем доступность API.")
...
def home():
    return Response("HTTPX Бот SofaScore работает напрямую!", status=200)
