...
# --- ИНИЦИАЛИЗАЦИЯ FLASK ДЛЯ RENDER (ДОЛЖНА БЫТЬ ТУТ) ---
app = Flask(__name__)
...
def make_request(url):
    # Тестируем БЕЗ прокси напрямую с серверов Render
...
def monitor_sts_table_tennis():
    ...
    send_telegram_message("🤖 <b>Бот успешно перезапущен на STS.pl!</b>\nТестируем прямые запросы без прокси.")
    
    # URL live-линии настольного тенниса на STS
        ...
        try:
            ...
            if data and "events" in data:
                ...
                for event in events:
                    ...
                    # Условие: равный счет по сетам 1:1
...
@app.route('/')
def home():
    return "Парсер линии STS активен и работает без прокси."
