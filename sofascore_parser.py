...
def make_request(url):
    ...
    try:
        print(f"[Сеть] Запрос к {url}...", flush=True)
...
def monitor_table_tennis():
    print("=== [ПОТОК] ТЕСТОВЫЙ ЗАПУСК БЕЗ ФИЛЬТРОВ ===", flush=True)
    send_telegram_message("🧪 <b>Тестовый режим запущен!</b>\nИщу вообще любые лайв-матчи настольного тенниса.")
        ...
        try:
            ...
            if matches:
                ...
                for match in matches:
                    ...
                    print(f"[Мониторинг] Отправляю тестовый матч ID {event_id}", flush=True)
                ...
                print("[Мониторинг] Лайв-матчи не найдены или API вернул пустой список.", flush=True)
