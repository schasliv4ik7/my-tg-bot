...
# --- WEB SERVER ДЛЯ RENDER (ДОЛЖЕН БЫТЬ НА ВЕРХНЕМ УРОВНЕ) ---
app = Flask(__name__)
...
def send_telegram_message(text):
    ...
    try:
        # Отправляем напрямую без прокси (на Render к Telegram стучится без проблем)
...
def get_sofascore_live():
    ...
    try:
        print("[SofaScore Сеть] Делаем запрос к Live API через прокси...", flush=True)
        response = client.get(url, headers=headers)
        print(f"[SofaScore Сеть] Ответ сервера: {response.status_code}", flush=True)
        
        if response.status_code == 200:
            return response.json()
        return None
    except Exception as e:
        print(f"[SofaScore Сеть] Ошибка подключения через прокси: {e}", flush=True)
        return None


def monitor_sofascore():
    time.sleep(5)
    print("=== [ПОТОК] СТАРТ МОНИТОРИНГА SOFASCORE (ЧЕРЕЗ ПРОКСИ) ===", flush=True)
    send_telegram_message("📱 <b>Бот мониторинга SofaScore успешно запущен через прокси!</b>\nНачинаем парсинг live-матчей.")
    ...
    while True:
        try:
            ...
            if events:
                print(f"[Мониторинг] Найдено матчей в лайве: {len(events)}", flush=True)
                ...
                for event in events:
                    ...
                    print(f"[Мониторинг] Найдена игра {home_name} - {away_name}. Отправляю...", flush=True)
                ...
                print("[Мониторинг] На SofaScore сейчас 0 активных лайв-игр или пустой ответ.", flush=True)
...
def home():
    return Response("HTTPX Бот SofaScore работает через прокси Webshare!", status=200)

def start_monitoring():
    for thread in threading.enumerate():
        if thread.name == "SofaScoreMonitor":
            return
    print("[Система] Инициализация фонового потока...", flush=True)
    monitor_thread = threading.Thread(target=monitor_sofascore, name="SofaScoreMonitor", daemon=True)
